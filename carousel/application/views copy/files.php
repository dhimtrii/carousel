...
<head>
   ...
   <script>
	 ...
	 $(".nameField").html(localStorage.getItem("fname")+ " " + localStorage.getItem("lname"));
            $.post("viewFile/files", { "patientId": localStorage.getItem("patientid") },
                        function(data){
...
<body>
	...
	<section id="main" class="column mainDiv">
		...
		<article class="module width_full stats" style="height:80%;overflow:auto;">
                     <header style="margin-top: -25px;"><h1 style="text-align:center;">Uploaded Files</h1></header>
                        <div class="fr" style="width:430px;margin-top: 30px;">
				 <div style="width: 300px; font-size: 16px; height: 35px; padding-top: 4px;" class="fl">Doctor Name:
				    <select style="font-size:16px;" id="selectDoc" class="selectDoc"></select></div>
                                 <div style="float:left;padding: 2% 5%;" id="shareDoc" class="medicationSaveButton blueButton">Share</div>
                                 </div>   
                        <div id="files"></div>
               </article>
        </section>
